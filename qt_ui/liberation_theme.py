...
def init():
    ...
    if os.path.isfile(THEME_PREFERENCES_FILE_PATH):
        try:
            with open(THEME_PREFERENCES_FILE_PATH, encoding="utf-8") as prefs:
...
def save_theme_config():
    ...
    with open(THEME_PREFERENCES_FILE_PATH, "w", encoding="utf-8") as prefs:
        prefs.write(json.dumps(pref_data, ensure_ascii=False))
