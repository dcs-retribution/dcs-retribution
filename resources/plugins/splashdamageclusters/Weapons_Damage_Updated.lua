...
function weaponDamage(bdaMessages, clusterEffects, shipRadarDamage)
  env.info(string.format("Splash Damage with Clusters and Ship Radar effects plugin running. BDA messages enabled: %s, Cluster munition damage updates enabled: %s, Ship radar damage enabled: %s",tostring(bdaMessages),tostring(clusterEffects),tostring(shipRadarDamage)))
