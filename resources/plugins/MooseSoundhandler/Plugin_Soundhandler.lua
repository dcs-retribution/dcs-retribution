...
if dcsRetribution then
    if dcsRetribution.plugins then
        ...
        ShipSamSounds = true
        PlayRedShootingGuns = true
        SoundDebug = false

        if dcsRetribution.plugins.MooseSoundhandler then
            --UPDATE WITH UI Preferences
            ...
            PlayRedShootingGuns = dcsRetribution.plugins.MooseSoundhandler.PlayRedShootingGuns
...
env.info("--------- SoundsToGroupOnly=" .. tostring(SoundToGroupOnly) ..
    ...
    if SoundHandler then
        ...
        if ShipSamSounds then
            if EventData.IniCoalition == 1 and
                ((GROUP:FindByName(EventData.IniGroupName):IsGround() and GROUP:FindByName(EventData.IniGroupName):IsSAM()) or
            ...
            if EventData.IniCoalition == 1 and (GROUP:FindByName(EventData.IniGroupName):IsGround() and GROUP:FindByName(EventData.IniGroupName):IsSAM()) and
...
if SoundDebug then BASE:I("-----GUN SHOOTING SOUNDS SET------") end
...
if SoundDebug then BASE:I("-----SOUNDHANDLER SETTING SET------") end

-----------------------------------------------------------------------------------------------------------------------------------
-- LOGGING UTILITIES
-----------------------------------------------------------------------------------------------------------------------------------

-- Log when a unit fires a shot
