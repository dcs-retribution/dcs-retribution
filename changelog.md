...
* **[Plugins]** Splash Damage's "Cluster Munitions Bomblets Count Reduction Modifier" toggle had no effect: the config layer assigned `cluster_bomblet_reduction_modifier` while the script reads `cluster_bomblet_reductionmodifier`, so the option was written to a key nothing ever read. The setting's mnemonic is unchanged, so saved plugin settings keep working.
