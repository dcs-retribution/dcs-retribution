...
* **[Mission Generator]** Warships can now fire real cruise missile strikes. Ships with land-attack cruise missiles (the vanilla Arleigh Burke's Tomahawks — carrier escorts included — and the CurrentHill packs' Kalibr hulls) get an F10 "Cruise Missile Strike" menu: put a map marker on a shore target and the nearest capable ship ripples a salvo at it (put just a number in the marker's text — "6" or "#6" — to fire exactly that many). Optional auto raids make each side commit one salvo a turn at its best reachable high-value ground target (command centers and comms first), announced to the defender only as a LAUNCH WARNING. The missiles are real weapons from real, sinkable ships — kills count at debrief, point-defense SAMs can intercept them — and every ship carries a finite campaign magazine with no rearm (a "Magazine status" menu shows what's left). Your side's tasked raid and ship magazines appear in the mission briefing, the naval group's info dialog shows its remaining missiles, and the debriefing window reports what was expended. New settings `Ship-launched cruise missile strikes` + `Auto-plan cruise missile raids` (Mission Generator, both default off; keep the new `Cruise missile strikes` plugin enabled).
