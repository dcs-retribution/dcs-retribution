...
* **[UX]** The squadron's country is now selectable in the Air Wing Configuration dialog (under Livery), and campaigns can pin it per squadron with `country:` — a coalition (CJTF) wing stops rolling a random nation's preset (voice, pilot names) for airframe-name squadron configs. Squadron preset dropdowns show each preset's nation, and Save/Load Config round-trips the country.
