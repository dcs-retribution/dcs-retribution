...
* **[Mission Generator]** Fixed-wing troop transports can now fly Air Assault missions by CTLD paradrop: any troop-capable transport (base-game C-47 and C-130, official C-130J-30 module, Anubis Hercules mod) plans the mission with troops preloaded and a run-in over the target zone. A player "Unload / Extract Troops" while airborne jumps the stick (troops land after a real descent and assault the zone); AI transports release automatically over their target zone. Grounded unloads and all helicopter behavior are unchanged, and troop preloading now retries until delayed (late-activated) transports actually spawn.
* **[Units]** The base-game C-47 and C-130 gain a troop cabin and an Air Assault priority, so AI transports can fly paradrop assaults without the Hercules mod or the C-130J-30 module. Their priorities sit below both, so either of those wins the tasking where available. The C-47 makes airborne assault available to the WWII factions, which field no assault helicopters.
