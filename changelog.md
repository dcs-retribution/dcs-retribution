...
## Fixes
* **[Kneeboard]** `utc_kneeboard` now does something. It was set on the Hornet family, the EA-18G and the VSN F-35s but read by nothing, so those cards printed local times with no Zulu anywhere. Every airframe that declares it now prints each time's Zulu equivalent on a second line, in the flight plan and the support tables, so the card serves both the jet and a squadron coordinating in local time. The F-16C is added to that list, whose System Time and CRUS TOS page run Zulu.
